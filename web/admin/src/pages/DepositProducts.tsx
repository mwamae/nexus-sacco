...
// All seven product types share one form; product_type-specific fields
// (lock-in, window months, default term) are surfaced conditionally.
...
export default function DepositProducts() {
  ...
  async function reload() {
    setErr(null);
    try { setProducts(await listDepositProducts(true)); }
...
function ProductModal({ initial, currency, onClose, onSaved }: {
  ...
  const [productType, setProductType] = useState<DepositProductType>(initial?.product_type ?? 'ordinary');
  ...
  function payload(): Partial<DepositProduct> {
    return {
      ...
      partial_withdrawal_allowed: partial,
    ...
    >
      <div className="card" style={{ width: 720, maxWidth: '92vw', maxHeight: '92vh', overflow: 'auto' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="card-body">
          ...
          <div className="grid-2" style={{ gap: 10 }}>
            ...
            <Field label="Type" hint={isNew ? undefined : 'Immutable after creation.'}>
              <select className="input" disabled={!isNew} value={productType} onChange={(e) => setProductType(e.target.value as DepositProductType)}>
                {(['ordinary', 'fixed', 'junior', 'holiday', 'goal', 'emergency', 'group'] as DepositProductType[]).map((p) => (
          ...
          {(productType === 'holiday' || productType === 'emergency') && (
          ...
          {(productType === 'fixed' || productType === 'goal') && (
            <>
