// Audit-log writes. Best-effort: callers log if Insert fails but
// never block. Writes to the shared audit_log table owned by identity.
// The table has no RLS so we don't need a tenant tx.
//
// Mirrors services/member/internal/store/audit_store.go.
