-- Fix mpesa_credentials_write: alias the OUT columns + the target
-- table so RETURNING isn't ambiguous between the function's output
-- columns and the table's columns. The body shipped in 0011 had
-- unaliased RETURNING which blew up with SQLSTATE 42702 at first
-- call. 0011 stays in the history so we don't rewrite applied
-- migrations; 0012 is the forward fix.
--
-- (Original 0011 rationale, kept verbatim for context.)
--
