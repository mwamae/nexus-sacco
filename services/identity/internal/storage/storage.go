...
type Storage interface {
	// Save reads from src and writes a file under tenant[/subject]/kind.
	// Pass uuid.Nil for subjectID when the file is owned by the tenant
	// itself (e.g. the branding logo).
	// Returns an opaque storage path that Open can later resolve.
	Save(tenantID, subjectID uuid.UUID, kind, mimeType string, src io.Reader, sizeHint int64) (path string, size int64, err error)
...
func (d *LocalDisk) Save(tenantID, subjectID uuid.UUID, kind, mimeType string, src io.Reader, _ int64) (string, int64, error) {
	var rel string
	if subjectID == uuid.Nil {
		rel = filepath.Join(tenantID.String(), kind+extFromMIME(mimeType))
	} else {
		rel = filepath.Join(tenantID.String(), subjectID.String(), kind+extFromMIME(mimeType))
	}
