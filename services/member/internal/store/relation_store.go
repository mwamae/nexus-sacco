...
func (s *RelationStore) ReplaceTx(ctx context.Context, tx pgx.Tx, tenantID, memberID uuid.UUID, kind domain.RelationKind, rows []RelationInput) error {
	if _, err := tx.Exec(ctx, `DELETE FROM member_relations WHERE member_id = $1 AND kind = $2`, memberID, kind); err != nil {
		return err
	}
	for i, r := range rows {
		if _, err := tx.Exec(ctx, `
			INSERT INTO member_relations
			  (member_id, tenant_id, kind, full_name, relationship, phone, email, id_doc_number, share_percent, position)
			VALUES
			  ($1, $2, $3, $4, $5, NULLIF($6,''), NULLIF($7,''), NULLIF($8,''), $9, $10)
		`, memberID, tenantID, kind, r.FullName, r.Relationship, r.Phone, r.Email, r.IDDocNumber, r.SharePercent, i); err != nil {
			return err
		}
	}
	return nil
...
func (s *RelationStore) ListForMemberTx(ctx context.Context, tx pgx.Tx, memberID uuid.UUID) ([]*domain.Relation, error) {
	rows, err := tx.Query(ctx, `
		SELECT id, member_id, kind, full_name, relationship,
		       COALESCE(phone,''), COALESCE(email,''), COALESCE(id_doc_number,''),
		       share_percent, position
		FROM member_relations WHERE member_id = $1 ORDER BY kind, position
	`, memberID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.MemberID, &r.Kind, &r.FullName, &r.Relationship,
