...
//
// Every cash-touching savings/loans/shares handler calls Post() after
// its own business writes have committed. The accounting service is
// idempotent on (source_module, source_ref) so a retry after a
// transient network failure won't double-post — but the handler still
// rolls back its own work if posting returns an error, ensuring the
// "no transaction is financially complete without a GL entry"
// invariant from the SACCO accounting spec.
