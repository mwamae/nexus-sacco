...
func (s *LoanProductStore) UpdateTx(ctx context.Context, tx pgx.Tx, p *domain.LoanProduct) (*domain.LoanProduct, error) {
	row := tx.QueryRow(ctx, `
		UPDATE loan_products SET
			...
			required_document_kinds = $26,
			min_membership_months = $27, min_shares_required = $28, allow_concurrent = $29,
			workflow_definition_code = $30, auto_approval_threshold = $31, auto_approval_min_score = $32,
			allow_topup = $33, allow_refinance = $34,
			updated_at = now()
		WHERE id = $1
		RETURNING `+loanProductCols,
		p.ID,
